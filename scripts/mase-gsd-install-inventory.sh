...
function branchTip(forkPath) {
  return run(['git', '-C', forkPath, 'rev-parse', 'refs/heads/mase/local-fixes'])
    || run(['git', '-C', forkPath, 'rev-parse', 'HEAD'])
    || '';
}
...
for (const { configDir, runtime } of configDirs) {
  ...
  const packageName = marker && !marker.__read_error
    ? marker.package_name || marker.packageIdentity?.packageName || marker.package_identity?.package_name || marker.package || 'unknown'
    : 'unknown';
  const binName = marker && !marker.__read_error
    ? marker.bin_name || marker.packageIdentity?.binName || marker.package_identity?.bin_name || marker.bin || 'unknown'
    : 'unknown';
  ...
  rows.push({
    ...
    profile: marker && marker.profile ? marker.profile : marker && marker.mode === 'minimal' ? 'legacy-minimal' : 'unknown',
