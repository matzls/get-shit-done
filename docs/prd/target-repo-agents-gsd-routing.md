...
- Prefer invoking installed GSD skills directly, such as `gsd-map-codebase` and
  `gsd-new-project`. Inspect workflow files under
  `.codex/get-shit-done/workflows/` only as fallback/debug evidence, not as the
  normal entrypoint.
