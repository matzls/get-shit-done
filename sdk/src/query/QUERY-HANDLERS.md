...
| `validate.agents` | No args; `agents_dir` follows runtime-aware resolution (`GSD_AGENTS_DIR`, else detected runtime config directory such as `$CODEX_HOME/agents` for `GSD_RUNTIME=codex`). `MODEL_PROFILES` / `expected` list stays aligned with `get-shit-done/bin/lib/model-profiles.cjs`. |
