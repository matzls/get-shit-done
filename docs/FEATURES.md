...
**Requirements:**
- REQ-GRAPH-01: Opt-in via `graphify.enabled: true` in `.planning/config.json`. When disabled, `/gsd-graphify build`, `query`, and `diff` print an activation hint and stop without writing; `/gsd-graphify status` remains read-only and reports Graphify context availability.
- REQ-GRAPH-02: Slash-command `/gsd-graphify` exposes subcommands `build`, `query <term>`, `status`, `diff`. The programmatic CLI `node gsd-tools.cjs graphify …` additionally exposes `snapshot`, which is also invoked automatically as the final step of `graphify build`, and `context-status`, an ungated read-only advisory check for Graphify context availability.
