...
<step name="plan_review_gate" required="true"> Advisory pre-execution gate: read `workflow.plan_review` with `gsd-sdk query config-get workflow.plan_review --default true`; discover existing `REVIEWS.md` / `*-REVIEWS.md` via `find "${PHASE_DIR}" -maxdepth 1 -type f \( -name '*-REVIEWS.md' -o -name 'REVIEWS.md' \) -print | sort | sed -n '1p'`; when enabled and no review exists, invoke `Skill(skill="gsd-review", args="--phase ${PHASE_NUMBER}")`, rediscover, report success or advisory miss, and always continue to `cross_ai_delegation`. </step>
